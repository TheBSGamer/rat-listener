...
function moveEmbed(client,oldState,newState,oldChannelId,newChannelId,logChannelMembersOnMove,messageToEdit,discordEpoch){
    let errorCheck = null;
    ...
    if (newState.channel?.members && logChannelMembersOnMove){
        ...
        if (newState.channel.members.size > 1){
            moveEmbed.setDescription(`<@${newState.member.user.id}> moved from <#${oldChannelId}> to <#${newChannelId}>\nID: ${newState.member.user.id}\n\nThe following users were members of the call:`);
        }
        for (let [memberId,guildMember] of newState.channel.members){
